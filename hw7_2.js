...
if (totalPlayers == 0)
    ...
    // spread опреатор (...), позволяющий развернуть данные из результирующего массива всех очков игроков 
    // в метод объекта Math.
        winnerId = scores.indexOf(Math.max(...scores))

    console.log(`Winner's id = ${players[winnerId].id}.`)
    console.log(`Winner's name is ${players[winnerId].name}.`) 
    console.log(`Winner's scorepoints = ${players[winnerId].scorePoints}.`)
    }
